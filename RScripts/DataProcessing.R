...
survey_list_orig <- lapply(data_dir[2:length(data_dir)], function(i) {
  
  #read in data
  x <- read_sav(list.files(i, pattern = ".sav", full.names = TRUE))
  return(x)
  
})

names(survey_list_orig) <- paste("Survey", 1:length(survey_list_orig), sep = "_")
names(survey_list_orig$Survey_4)[names(survey_list_orig$Survey_4) == "provin"] <- "province"

# for the sake of google translate
#i <- 1
#sapply(surveyQuestion_per_round, function(x) {
#  
#  write.table(x, file = paste0("surveyRound",i, ".txt"), row.names = FALSE)
#  i <<- i+1
#  
#})

### clean the factor columns
survey_list <- lapply(1:length(survey_list_orig), function(i) {
  
  
  x <- survey_list_orig[[i]]
  
  if(i == 4) {
    
    names(x)[which(names(x) == "provin")] <- "province"
    
  }
  
  if(i == 1) {
    
    x <- x %>% mutate(Diploma = create_diploma(Q27))
    
  } else {
    
    x <- x %>% mutate(Diploma = create_diploma(Q6))
    
  }
  y <- convert_label(x)
  levels(y$province) <- vbrant(y$province)
  return(y)
...
surveyQuestion_per_round <- lapply(survey_list_orig, function(x) {
